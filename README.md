...
| POST   | `/password-reset-requests`        | Submit a phone number for the admin to review *(public, rate-limited, same response whether or not the phone has an account)* |
| GET    | `/admin/password-reset-requests`  | List pending (or all) reset requests, requires `ADMIN_RESET_SECRET`        |
| POST   | `/admin/password-reset-requests/{request_id}/resolve` | Mark a reset request resolved without necessarily resetting the password, requires `ADMIN_RESET_SECRET` |
...
- There's no budget for a paid email/SMS provider, and the app authenticates by phone number rather than email, so a self-service "forgot password" flow isn't buildable for free right now. Instead, a locked-out user submits their phone number (and optionally a short message) via `POST /password-reset-requests` — this is public and rate-limited, and always returns the same response regardless of whether that phone number has an account, to avoid leaking who's registered. The admin reviews these through a small dashboard (`/static/admin.html`, backed by `GET /admin/password-reset-requests`), verifies the person's identity out-of-band, and either resets the password via `POST /admin/reset-password` (which also revokes all of that user's existing sessions) or marks the request resolved without a reset if it turns out to be invalid or unresolvable. All three admin endpoints are gated behind `ADMIN_RESET_SECRET`, entered per-session in the admin page (never persisted to localStorage) and never returned to non-admin clients. This is still an interim, operator-run process rather than a finished self-service flow — revisit if the user base grows past what one admin can handle manually.
...
- This project is still a work in progress. This is aimed to be deployed in about two years from now for a demo proposal.